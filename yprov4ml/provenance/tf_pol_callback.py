...
class POLLoggingCallback(tf.keras.callbacks.Callback):
    def __init__(self, model_ptr, log_every_n_steps=5):
    ...
    def on_train_batch_end(self, batch, logs=None):        
        ...
        if self.step_counter % self.log_every_n_steps == 0: 
            yprov4ml.log_proof_of_learning_step(self.model, logs["loss"], batch, step=self.current_epoch, context=yprov4ml.Context.TRAINING)
        else: 
            self.step_counter += 1